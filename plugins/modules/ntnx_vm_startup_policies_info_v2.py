...
short_description: Fetches information about Nutanix PC VM startup policies.
version_added: "2.6.0"
author:
 - Abhinav Bansal (@abhinavbansal29)
description:
  - This module fetches information about Nutanix PC VM startup policies.
...
- name: Get all VM startup policies
  nutanix.ncp.ntnx_vm_startup_policies_info_v2:
    nutanix_host: "{{ ip }}"
    nutanix_username: "{{ username }}"
    nutanix_password: "{{ password }}"
...
response:
  description:
    - The response from the Nutanix PC VM startup policies API.
...
msg:
    ...
    type: str
    sample: "Api Exception raised while fetching VM startup policy info"
error:
...
total_available_results:
    description:
        - The total number of available VM startup policies in PC.
    type: int
...
def get_policy(module, result):
    """Fetch a single VM startup policy by ext_id."""
    policies = get_vm_startup_policies_api_instance(module)
    ext_id = module.params.get("ext_id")

    try:
        resp = policies.get_vm_startup_policy_by_id(extId=ext_id)
    except Exception as e:
        raise_api_exception(
            module=module,
            exception=e,
            msg="Api Exception raised while fetching VM startup policy info",
        )
...
def get_policies(module, result):
    """Fetch a list of VM startup policies with pagination."""
    policies = get_vm_startup_policies_api_instance(module)
